...
@section('content')
    @php
        $all_income = App\Models\Income::select(DB::raw('count(*) as total'), DB::raw('YEAR(income_date) year, MONTH(income_date) month'))
            ->groupby('year', 'month')
            ->orderBy('income_date', 'DESC')
            ->get();
        $all_expense = App\Models\Expense::select(DB::raw('count(*) as total'), DB::raw('YEAR(expense_date) year, MONTH(expense_date) month'))
            ->groupby('year', 'month')
                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-md-4 card_button_part">
                            <a href="{{ url('dashboard/income') }}" class="btn btn-sm btn-dark"><i class="fas fa-th"></i>All
                                Income</a>
                            <a href="{{ url('dashboard/expense') }}" class="btn btn-sm btn-dark"><i
                                    class="fas fa-th"></i>All Expense</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                                        ...
                                        <div class="btn-group btn_group_manage" role="group">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a class="dropdown-item"
                                                        href="{{ url('dashboard/archive/' . $month_year) }}">Details</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
